...
int saveFlightToFileBit(const Flight* pF, FILE* fp)
{
	...
	data[1] = pF->date.year << 6 | pF->date.month << 2 | pF->date.day >> 3;
	data[0] = pF->date.day << 5 | pF->hour;
...
int loadFlightFromFileBit(Flight* pF, FILE* fp, const AirportManager* pManager)
{
	char origin[CODE_LENGTH + 1] = { "" }, dest[CODE_LENGTH + 1] = { "" };
	if (fread(origin, sizeof(char), CODE_LENGTH, fp) != CODE_LENGTH)
		return 0;
	if (fread(dest, sizeof(char), CODE_LENGTH, fp) != CODE_LENGTH)
		return 0;
	origin[CODE_LENGTH] = '\0';
	dest[CODE_LENGTH] = '\0';
	if (checkUniqeCode(origin, pManager) && checkUniqeCode(dest, pManager))
		return 0;
	strcpy(pF->originCode, origin);
	strcpy(pF->destCode, dest);
	BYTE data[4] = { 0 };
	if (fread(data, sizeof(BYTE), 4, fp) != 4)
		return 0;
	pF->date.year = (data[3] << 10) | (data[2] << 2) | ((data[1] >> 6) & 0x3);
	pF->date.month = (data[1] >> 2) & 0xF;
	pF->date.day = (data[1] & 0x3) << 3 | data[0] >> 5;
	pF->hour = data[0] & 0x1F;
	return 1;
}
